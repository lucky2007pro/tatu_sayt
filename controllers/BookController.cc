...
// ── Navbatga yozilish ─────────────────────────────────────────────────────────
void BookController::waitlist(const HttpRequestPtr& req,
                              std::function<void(const HttpResponsePtr&)>&& cb,
                              int id)
{
    if (!checkCsrf(req)) { cb(errResp(403, "CSRF xato")); return; }
    ...
    apiPost("/api/waitlist/", body,
        [req, cb = std::move(cb), backUrl](int status, const Json::Value& data) mutable {
            if (status == 200 || status == 201) {
                req->session()->insert("flash_msg",
                    std::string("Navbatga muvaffaqiyatli yozildingiz! Kitob bo'shaganda xabar beramiz."));
                req->session()->insert("flash_type", std::string("success"));
            } else {
                std::string err = "Navbatga yozilishda xato.";
                if (data.isObject() && data.isMember("detail") && data["detail"].isString())
                    err = data["detail"].asString();
                else if (data.isObject() && data.isMember("error") && data["error"].isString())
                    err = data["error"].asString();
                req->session()->insert("flash_msg",  err);
                req->session()->insert("flash_type", std::string("error"));
            }
...
// ── Navbatdan chiqish ─────────────────────────────────────────────────────────
void BookController::removeWaitlist(const HttpRequestPtr& req,
                                    std::function<void(const HttpResponsePtr&)>&& cb,
                                    int id)
{
    if (!checkCsrf(req)) { cb(errResp(403, "CSRF xato")); return; }
    ...
    apiDelete("/api/waitlist/?book=" + std::to_string(id),
        [req, cb = std::move(cb), backUrl](int status) mutable {
            if (status >= 200 && status < 300) {
                req->session()->insert("flash_msg",  std::string("Navbatdan chiqtingiz."));
                req->session()->insert("flash_type", std::string("success"));
            } else {
                req->session()->insert("flash_msg",  std::string("Navbatdan chiqishda xato."));
                req->session()->insert("flash_type", std::string("error"));
            }
            cb(redirect(backUrl));
        }, token);
